...
public class UglyCalculatorTest {
	...
	public static void main(final String[] args) {
		...
		if (Math.abs(expectedValue - actualValue) > expectedValue * maxRelativeError) {
			System.err.println("isqrt(" + radix + ") weicht zu stark vom erwarteten Wert (" + expectedValue + ") ab: "
					+ actualValue);
		}
		...
		if (Math.abs(expectedValue - actualValue) > expectedValue * maxRelativeError) {
			System.err.println(
					"isqrt(1E64) weicht zu stark vom erwarteten Wert (" + expectedValue + ") ab: " + actualValue);
		}

		radix = 0;
		float isqrt = calculator.isqrt(radix);
		if (isqrt != 1 / 0.F) {
			System.err.println("isqrt(0) gab nicht INF zurück!");
		}
